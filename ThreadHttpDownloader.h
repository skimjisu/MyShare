...
enum DOWN_ERR_CODE {
	DOWN_ERR_URLERR, DOWN_ERR_DOWNDONE, DOWN_ERR_NOERROR,
};
...
typedef void __fastcall(__closure * TThreadHttpDownloadWorkEvent)(System::TObject * Sender, int Tag, __int64 NowPos,
	__int64 TotalSize);
typedef void __fastcall(__closure * TThreadHttpDownloadStartEvent)(System::TObject * Sender, int Tag, __int64 StartPos,
	__int64 TotalSize);
typedef void __fastcall(__closure * TThreadHttpDownloadSpeedEvent)(System::TObject * Sender, int Tag, __int64 SpeedSize);
typedef void __fastcall(__closure * TThreadHttpDownloadRemainTimeEvent)(System::TObject * Sender, int Tag,
	AnsiString RemainTimeString);
...
public:
	...
	TIdHTTP *HTTPC;
	TTimer *SpeedCheckTimer;    // 속도 체크 타이머
	AnsiString NowDownLoadFilePath;
	AnsiString NowDownLoadFileURL; // 다운로드 주소
